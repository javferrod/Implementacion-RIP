...
public class Packet {
    ...
    Packet(Tipo t, int numEntradas){
       ...
       /*-----HEADER----*/
        mensaje.put(0, (byte) t.v);                      //Command
    ...
    ArrayList<Entry> getEntrys(){
        ...
        for (int j = 0; j < (mensaje.limit()-4)/20; j++) {
            if(mensaje.get(j * 20 + 8)==(byte)0) return RIPentries; //Cuando no haya IPv4 significa que ya no hay más entradas
    ...
    void setMetric(int index, byte metric){
        mensaje.put(20*(index-1)+23,metric);
    }
